...
class Ticket
{
    ...
    public function getMoviePrice($movieType, $startDateTime) {
        $price = 0.00;
    ...
    public function isFridayDiscount($startDateTime) {
        $currentDay = date('l', strtotime($startDateTime));
    ...
    public function sellTickets($timeSlotId, $seat, $price, $userId) {
    ...
    public function getTicketById($ticketId) {
        ...
        try {
            ...
            return [
                'error' => true,
                'message' => "No ticket with id {$ticketId} wasfound."
            ];
    ...
    public function generatePDF($ticketId, $screenID, $startDateTime, $movie, $seats, $totalPrice) {
        $dompdf = new Dompdf();

        $startsAt = date('H:i l d-m-Y', strtotime($startDateTime));
        $movieTitle = $movie['title'];
        $movieType = $movie['type'];
        $fridayDiscount = $this->isFridayDiscount($startDateTime);
    ...
    public function getMostNrOfTicketsSold() {
        $pdo = Database::getConnection();
